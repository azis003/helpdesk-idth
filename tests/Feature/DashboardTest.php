...
class DashboardTest extends TestCase
{
    ...
    public function test_technician_dashboard_shows_personal_ticket_summary(): void
    {
        ...
        $this->actingAs($technician)
            ...
            ->assertDontSee('<dt>Dikerjakan Sendiri</dt>', false)
            ->assertDontSee('<dt>Dikerjakan Teknisi</dt>', false)
            ->assertDontSee('<dt>Antrian Tiket</dt>', false)
            ->assertDontSee('id="helpdesk-queue-heading"', false)
            ->assertSee('href="'.route('tickets.queue', ['tab' => 'mine']).'" class="hd-outline-button">Buka Tiket Saya</a>', false)
            ->assertSee('Tiket masih dikerjakan')
            ->assertSee('Tiket menunggu konfirmasi')
            ->assertSee('Tiket sudah ditutup')
            ->assertDontSee('Tiket milik teknisi lain')
