...
@section('content')
    <div class="ui-page-header">
        <div>
            <p class="ui-eyebrow"><span class="ui-eyebrow-dot" aria-hidden="true"></span>Komunikasi layanan</p>
            <h1 class="ui-page-title">Pengumuman layanan TI</h1>
            <p class="ui-page-description">Sampaikan kabar penting di dasbor dan sebelum pengguna membuat tiket. Atur waktunya agar pesan tetap relevan.</p>
        </div>
    </div>

    <section class="ui-announcement-hero mt-7" aria-labelledby="announcement-hero-heading">
    ...
    <section class="mt-5 grid gap-3 sm:grid-cols-3" aria-label="Ringkasan pengumuman">
        <article class="ui-stat-card ui-announcement-stat">
            ...
            <div><p class="ui-stat-label">Tampil sekarang</p><p class="ui-stat-value">{{ $activeCount }}</p><p class="ui-announcement-stat-meta">Terlihat sesuai jadwal aktif.</p></div>
        </article>
        <article class="ui-stat-card ui-announcement-stat">
            <span class="ui-stat-icon !bg-[#fff6df] !text-[#a16207]" aria-hidden="true"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="4.5" y="5" width="15" height="14" rx="1.5" /><path stroke-linecap="round" d="M8 3.5v3M16 3.5v3M7.5 10h9M8 14h3" /></svg></span>
            <div><p class="ui-stat-label">Menunggu jadwal</p><p class="ui-stat-value">{{ $scheduledCount }}</p><p class="ui-announcement-stat-meta">Akan tampil otomatis saat waktunya tiba.</p></div>
        </article>
        <article class="ui-stat-card ui-announcement-stat">
            <span class="ui-stat-icon !bg-[#eef3ff] !text-[#4f63a6]" aria-hidden="true"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M6 5h12M6 12h12M6 19h8" /><path stroke-linecap="round" d="M4 5h.01M4 12h.01M4 19h.01" /></svg></span>
            <div><p class="ui-stat-label">Semua pengumuman</p><p class="ui-stat-value">{{ $announcements->count() }}</p><p class="ui-announcement-stat-meta">Termasuk yang nonaktif dan selesai.</p></div>
        </article>
    </section>

    <section class="mt-7 grid items-start gap-5 xl:grid-cols-[minmax(20rem,0.82fr)_minmax(0,1.18fr)]">
        <form id="announcement-create" method="POST" action="{{ route('admin.announcements.store') }}" class="ui-panel overflow-hidden" aria-labelledby="announcement-create-heading">
            ...
            <div class="ui-panel-header">
                <p class="ui-eyebrow"><span class="ui-eyebrow-dot !bg-[#75d5f3] !shadow-[0_0_0_4px_#d9f6ff]" aria-hidden="true"></span>Mulai dari sini</p>
            ...
            <div class="space-y-5 p-5 sm:p-6">
                <div class="rounded-lg border border-[#dcebef] bg-[#f7fbfc] px-3.5 py-3 text-xs leading-5 text-[#55747f]">
                    <span class="font-extrabold text-[#355966]">Tip cepat:</span> satu pengumuman sebaiknya menyampaikan satu kabar utama.
                </div>

                <div>
                    <label for="announcement-title" class="ui-field-label">Judul <span class="text-rose-600" aria-hidden="true">*</span><span class="sr-only"> wajib</span></label>
                    ...
                    @if ($isCreateError)
                        @error('title')<p id="announcement-title-error" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                    @endif
                </div>

                <div>
                    <label for="announcement-body" class="ui-field-label">Isi pengumuman <span class="text-rose-600" aria-hidden="true">*</span><span class="sr-only"> wajib</span></label>
                    ...
                    @if ($isCreateError)
                        @error('body')<p id="announcement-body-error" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                    @endif
                </div>

                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label for="announcement-starts" class="ui-field-label">Mulai ditampilkan <span class="text-rose-600" aria-hidden="true">*</span><span class="sr-only"> wajib</span></label>
                        ...
                        @if ($isCreateError)
                            @error('starts_at')<p id="announcement-starts-error" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                        @endif
                    </div>
                    <div>
                        <label for="announcement-ends" class="ui-field-label">Berhenti ditampilkan <span class="font-normal text-[#78909a]">(opsional)</span></label>
                        ...
                        @if ($isCreateError)
                            @error('ends_at')<p id="announcement-ends-error" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                        @endif
                    </div>
                </div>

                <div>
                    <label class="flex min-h-12 items-start gap-3 rounded-lg border border-[#e1eaed] bg-[#f8fbfc] px-3 py-2.5 text-xs text-[#526f79]">
                        ...
                        <span><span class="block font-extrabold text-[#35505b]">Aktifkan sesuai jadwal</span><span class="mt-0.5 block leading-5 text-[#78909a]">Pengumuman akan tampil hanya dalam rentang waktu yang Anda tentukan.</span></span>
                    </label>
                    @if ($isCreateError)
                        @error('is_active')<p id="announcement-active-error" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                    @endif
                </div>

                <button type="submit" class="ui-btn ui-btn-primary w-full">Simpan pengumuman</button>

                <div class="border-t border-[#edf2f4] pt-4">
                    <p class="text-xs font-extrabold text-[#526f79]">Akan tampil di mana?</p>
                    <p class="mt-1 text-xs leading-5 text-[#78909a]">Pengumuman aktif muncul di dasbor dan di atas formulir pembuatan tiket.</p>
                </div>
            </div>
        </form>
        ...
        <section class="ui-panel overflow-hidden" aria-labelledby="announcement-list-heading">
            <div class="ui-panel-header flex flex-wrap items-start justify-between gap-3">
                <div>
                    <p class="ui-eyebrow"><span class="ui-eyebrow-dot !bg-[#ffd44f] !shadow-[0_0_0_4px_#fff0b9]" aria-hidden="true"></span>Yang sudah dibuat</p>
                ...
                <span class="ui-announcement-count">{{ $announcements->count() }} total</span>
            </div>
                    ...
                    <details class="ui-announcement-item" @if ($isEditError) open @endif>
                            ...
                            <span class="flex min-w-0 items-start gap-3">
                                ...
                                <span class="min-w-0">
                                    <span class="flex flex-wrap items-center gap-2">
                                        <span class="block max-w-full truncate text-sm font-extrabold text-[#35505b]">{{ $announcement->title }}</span>
                                        <span class="ui-status {{ $statusClass }}">{{ $statusLabel }}</span>
                                    </span>
                                    <span class="mt-1 block text-[0.68rem] leading-5 text-[#78909a]">Mulai {{ $startsAtLabel }} <span aria-hidden="true">&middot;</span> {{ $endsAtLabel ? 'Berakhir '.$endsAtLabel : 'Tanpa batas akhir' }} <span aria-hidden="true">&middot;</span> {{ $announcement->creator?->name ?? 'Pembuat tidak tersedia' }}</span>
                                </span>
                            </span>
                        </summary>
                        ...
                        <div class="border-t border-[#edf2f4] p-4 sm:p-5">
                            <div class="ui-announcement-preview">
                                ...
                                <p class="mt-2 whitespace-pre-line text-sm leading-6 text-[#526f79]">{{ $announcement->body }}</p>
                            </div>

                            <div class="mt-5 border-t border-[#edf2f4] pt-5">
                                <p class="ui-eyebrow"><span class="ui-eyebrow-dot !bg-[#75d5f3] !shadow-[0_0_0_4px_#d9f6ff]" aria-hidden="true"></span>Atur kembali</p>
                                <h3 class="mt-2 text-base font-extrabold tracking-tight text-[#263a43]">Perbarui pengumuman</h3>
                                <p class="mt-1 text-xs leading-5 text-[#78909a]">Perubahan jadwal dan isi tersimpan sebagai pengaturan terbaru untuk pesan ini.</p>
                                ...
                                <form method="POST" action="{{ route('admin.announcements.update', $announcement) }}" class="mt-4 space-y-4">
                                    ...
                                    <div>
                                        <label for="edit-announcement-title-{{ $announcement->id }}" class="ui-field-label">Judul <span class="text-rose-600" aria-hidden="true">*</span><span class="sr-only"> wajib</span></label>
                                        ...
                                        @if ($isEditError)
                                            @error('title')<p id="edit-announcement-title-error-{{ $announcement->id }}" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                                        @endif
                                    </div>

                                    <div>
                                        <label for="edit-announcement-body-{{ $announcement->id }}" class="ui-field-label">Isi pengumuman <span class="text-rose-600" aria-hidden="true">*</span><span class="sr-only"> wajib</span></label>
                                        ...
                                        @if ($isEditError)
                                            @error('body')<p id="edit-announcement-body-error-{{ $announcement->id }}" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                                        @endif
                                    </div>

                                    <div class="grid gap-4 sm:grid-cols-2">
                                        <div>
                                            <label for="edit-announcement-starts-{{ $announcement->id }}" class="ui-field-label">Mulai ditampilkan <span class="text-rose-600" aria-hidden="true">*</span><span class="sr-only"> wajib</span></label>
                                            ...
                                            @if ($isEditError)
                                                @error('starts_at')<p id="edit-announcement-starts-error-{{ $announcement->id }}" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                                            @endif
                                        </div>
                                        <div>
                                            <label for="edit-announcement-ends-{{ $announcement->id }}" class="ui-field-label">Berhenti ditampilkan <span class="font-normal text-[#78909a]">(opsional)</span></label>
                                            ...
                                            @if ($isEditError)
                                                @error('ends_at')<p id="edit-announcement-ends-error-{{ $announcement->id }}" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <label class="flex min-h-12 items-start gap-3 rounded-lg border border-[#e1eaed] bg-[#f8fbfc] px-3 py-2.5 text-xs text-[#526f79]">
                                            ...
                                            <span><span class="block font-extrabold text-[#35505b]">Aktifkan sesuai jadwal</span><span class="mt-0.5 block leading-5 text-[#78909a]">Matikan sementara jika pesan belum perlu terlihat atau sudah tidak relevan.</span></span>
                                        </label>
                                        @if ($isEditError)
                                            @error('is_active')<p id="edit-announcement-active-error-{{ $announcement->id }}" class="mt-2 text-sm text-rose-700">{{ $message }}</p>@enderror
                                        @endif
                                    </div>
                                ...
                                <div class="mt-5 flex flex-col gap-3 border-t border-[#edf2f4] pt-4 sm:flex-row sm:items-center sm:justify-between">
                                    <p class="text-xs leading-5 text-[#78909a]">{{ $announcement->is_active ? 'Nonaktifkan sementara tanpa menghapus isi dan jadwal.' : 'Aktifkan kembali agar jadwalnya dapat bekerja.' }}</p>
