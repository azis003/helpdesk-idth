...
@section('content')
    <div class="ui-page-header">
        <div>
            <p class="ui-eyebrow"><span class="ui-eyebrow-dot" aria-hidden="true"></span>Ruang kerja</p>
    ...
    <nav class="ui-ticket-tabs mt-6 flex flex-wrap items-center gap-2" aria-label="Filter Monitoring Tiket">
        @foreach ($tabs as $tabKey => $tab)
            @continue(! $tab['visible'])
            @php
                $isActiveTab = $activeTab === $tabKey;
            @endphp
            <a href="{{ route('tickets.queue', ['tab' => $tabKey]) }}" class="ui-ticket-tab {{ $isActiveTab ? 'is-active' : '' }}" @if ($isActiveTab) aria-current="page" @endif>
                <span>{{ $tab['label'] }}</span>
    ...
    <section class="ui-panel ui-list-panel mt-4 overflow-hidden" aria-labelledby="queue-heading">
